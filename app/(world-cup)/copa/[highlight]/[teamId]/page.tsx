...
export async function generateMetadata({ params, searchParams }: Props): Promise<Metadata> {
  ...
  const result = await request<RoundData>(ENDPOINTS.TEAM_ROUND(teamId, '1'), { isWorldCup: true })
...
export default async function TeamStatisticsStaticPage({ params, searchParams }: Props) {
  ...
  if (!searchParams.year) return redirect('/copa')

  const year = `${+searchParams.year}-world-cup` as const
  ...
  const rounds = [...SEASONS.CUP_2026.FIRST_TURN_ROUNDS, ...SEASONS.CUP_2026.SECOND_TURN_ROUNDS]

  const results = await Promise.allSettled<RoundData>(
    rounds.map((round) => {
      return getTeamRoundsData(teamId.toString(), round.toString(), year)
    })
  )
    ...
    <div className="w-full pb-4">
      ...
      <TeamStatisticsContent
        data={{
          ...
          teamInfo,
          rounds: roundsData,
          year: +searchParams.year as SeasonYears
        }}
      />
      <Signature />
    </div>
