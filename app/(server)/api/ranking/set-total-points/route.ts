...
import { insertTeamRankingData } from '../../repositories/total-points-ranking.database'
...
export async function GET(request: Request) {
  ...
  if (!isEmpty(teamsLatestRound)) {
    teamsLatestRound.forEach(async (teamRound) => {
      if (!isNil(teamRound.payload.pontos_campeonato))
        insertResponses.push(await insertTeamRankingData(teamRound.payload, year))
    })
  }
